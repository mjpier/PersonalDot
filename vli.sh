...
function install_base_system_and_chroot {
  ...
  XBPS_ARCH="$ARCH" xbps-install -Suvy -r /mnt -R "$REPO" base-minimal btrfs-progs bash less man-pages e2fsprogs pciutils usbutils iproute2 dhcpcd kbd ethtool kmod iputils xfsprogs f2fs-tools dosfstools traceroute ncurses acpid eudev file mdocml wpa_supplicant iw iwd NetworkManager ipw2100-firmware ipw2200-firmware zd1211-firmware linux-firmware-amd linux-firmware-broadcom linux-firmware-network linux-firmware-qualcomm linux6.3 linux6.3-headers dracut efibootmgr opendoas git cryptsetup grub-x86_64-efi lvm2 bash-completion nano gcc apparmor curl util-linux tar coreutils binutils xtools fzf plocate ictree xkeyboard-config ckbcomp void-repo-nonfree
